...
const AddressCard = ({ addr, onEditHandler, setAddresses: setAddresses }: AddressCardProps) => {

    const { updateUser } = useAuth()
    const handleDelete = async (id: string) => {
        try {
            const confirm = window.confirm("Are you sure you want to delte this address")
            if (!confirm) return
